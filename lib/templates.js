...
const REVIEW_AVG = (reviews.reduce((a, r) => a + r.rating, 0) / REVIEW_COUNT).toFixed(1);
...
function pricingCardsHtml() {
  const cards = site.pricing.map((p) => `
    <div class="price-card${p.featured ? ' price-card--featured' : ''}">
      ...
      <div class="price-card__price">${esc(p.price)}<small>원</small></div>
      <div class="price-card__time">${esc(p.time)}</div>
      <p class="price-card__desc">${esc(p.desc)}</p>
      <a class="btn ${p.featured ? 'btn--accent' : 'btn--ghost'}" href="${site.phoneHref}">예약 문의</a>
    </div>`).join('');
  return `<h2 id="pricing">마사지 코스와 요금</h2>
<div class="pricing-grid pricing-grid--inline">${cards}</div>
<p class="pricing-note">코스별 기준 요금은 경기도 전 지역 동일하게 안내되며, 추가 비용 없이 있는 그대로 안내해 드립니다. ${site.pricingNote} <a href="/check/travel-fee/">상세 요금 안내 보기 →</a></p>`;
}

// 이전 이름 호환 — 전 페이지에서 pricingNoteHtml() 호출 시 가격표 카드로 렌더링
function pricingNoteHtml() { return pricingCardsHtml(); }
...
// 히어로 이미지 스타일 — heroImage가 설정되면 항상 참조(정적 배포에서 파일만 올리면 즉시 반영).
// 파일이 아직 없으면 CSS 오버레이+배경색으로 자연 폴백된다.
function heroBg() {
  const img = site.heroImage;
  if (!img) return { on: false, style: '' };
  return { on: true, style: ` style="--hero-img:url('${img}')"` };
}

// 지역·상세 페이지 상단 히어로 (배경 이미지 + 브레드크럼 + H1 + 리드 + CTA)
function pageHero(crumbs, h1, leadHtml, opts = {}) {
  const bg = heroBg();
  const cls = 'hero hero--page' + (bg.on ? ' hero--image' : '');
  const cta = opts.cta === false ? '' : ctaHtml();
  return `<section class="${cls}"${bg.style}>
  <div class="container">
    ${breadcrumbHtml(crumbs)}
    <h1>${h1}</h1>
    ${leadHtml ? `<p class="hero__lead">${leadHtml}</p>` : ''}
    ${cta}
  </div>
</section>`;
}

// 고객 후기 섹션 (전 페이지 하단 공용) — limit로 노출 개수 조절
function reviewsSectionHtml(limit) {
  const list = (limit ? reviews.slice(0, limit) : reviews);
  const cards = list.map((r) => `
    <li class="review-card">
      <div class="review-card__top">
        <span class="review-card__stars" aria-label="별점 ${r.rating}점 만점 5점">${stars(r.rating)}</span>
        <span class="review-card__place">${esc(r.place)}</span>
      </div>
      <p class="review-card__body">${esc(r.body)}</p>
      <div class="review-card__meta">${esc(r.author)} · ${esc(r.date)}</div>
    </li>`).join('');
  return `<section class="section section--reviews" id="reviews" aria-labelledby="reviews-h">
  <div class="container">
    <div class="section-head">
      <h2 id="reviews-h">고객 후기</h2>
      <p><span class="review-agg__stars">${stars(Math.round(REVIEW_AVG))}</span> 평균 ${REVIEW_AVG} / 5 · 후기 ${REVIEW_COUNT}건 · 실제 이용 고객이 남긴 후기입니다.</p>
    </div>
    <ul class="review-grid">${cards}</ul>
    <p class="reviews-more"><a href="/reviews/">고객 후기 전체 보기 →</a></p>
  </div>
</section>`;
}

// Review + AggregateRating 구조화 데이터 (Service 기준) — 화면 노출 후기와 일치
function reviewSchema() {
  return {
    '@context': 'https://schema.org',
    '@type': 'Service',
    name: `${site.brand} 경기도 출장마사지`,
    serviceType: '출장마사지',
    areaServed: '경기도',
    provider: {
      '@type': 'Organization', name: site.brand, url: site.siteUrl, telephone: site.phone,
    },
    aggregateRating: {
      '@type': 'AggregateRating',
      ratingValue: REVIEW_AVG, reviewCount: String(REVIEW_COUNT), bestRating: '5', worstRating: '1',
    },
    review: reviews.map((r) => ({
      ...
      reviewBody: r.body,
      itemReviewed: { '@type': 'Service', name: `${site.brand} 경기도 출장마사지 · ${r.place}` },
    })),
  };
}

// 전체 페이지 레이아웃
function layout({ title, desc, path, schemas = [], activePath = null, noindex = false, reviewsSection = true }, body) {
  const description = d80(desc);
  const canonical = site.siteUrl + path;
  // 후기 섹션을 넣는 페이지에만 Review/AggregateRating 스키마를 함께 넣어 화면 노출과 일치시킴
  const allSchemas = reviewsSection ? [...schemas, reviewSchema()] : schemas;
...
<head>
  ...
  <link rel="alternate" type="application/rss+xml" title="${site.brand} 업데이트" href="/rss.xml">
...
<main id="main">
${body}
${reviewsSection ? reviewsSectionHtml(6) : ''}
</main>
${footerHtml()}
</body>
</html>`;
...
module.exports = {
  ...
  bookingFlowHtml, pricingNoteHtml, pricingCardsHtml, hubIntroHtml, pageHero, heroBg,
  reviewsSectionHtml, reviewSchema, stars, REVIEW_AVG, REVIEW_COUNT,
  site, reviews,
};
